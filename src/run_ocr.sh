...
#parallel --bar -j $num_cores ./text_blob.py
     echo ------------------------using thread 1
     perf stat -I 6000 -e L1-dcache-load-misses,instructions,l2_rqsts.miss,branch-misses,cycles -x, -o $input_t1_csv python text_blob.py $t1_f1 8
...
then
     echo -------------------------using thread 2
     perf stat -I 3000 -e L1-dcache-load-misses,instructions,l2_rqsts.miss,branch-misses,cycles -x, -o $input_t2_csv parallel ::: "python text_blob.py $t1_f1 4" "python text_blob.py $t1_f1 4"
     interval-normalize.py $input_t2_csv > $output_t2_csv
     Rscript plot2.R    

     echo -------------------------using thread 4
     perf stat -I 1000 -e L1-dcache-load-misses,instructions,l2_rqsts.miss,branch-misses,cycles -x, -o $input_t4_csv parallel ::: "python text_blob.py $t1_f1 2" \
     ...
     echo ------------------------ using thread 8
     perf stat -I 1000 -e L1-dcache-load-misses,instructions,l2_rqsts.miss,branch-misses,cycles -x, -o $input_t8_csv parallel ::: "python text_blob.py $t1_f1 1" \
